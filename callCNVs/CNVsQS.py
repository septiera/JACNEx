...
def getCNVsQualityScore(CNVs, CNProbs_A, CNProbs_G):
    quality_scores = []
    for CNV in CNVs:
        score = calculateCNVQualityScore(CNV, CNProbs_A, CNProbs_G)
...
#                    dim = NbExonsOnAutosomes * NbOfCNStates
# - CNPath_G (dict): same as CNProbs_A but for gonosomes
def calculateCNVQualityScore(CNV, CNProbs_A, CNProbs_G):
    # Extract necessary information from the CNV
    chromType, exonStart, exonEnd, pathProb, sampleName = CNV[0], CNV[2], CNV[3], CNV[4], CNV[5]

    # Select the appropriate probabilities dictionary
    CNProbs = {'A': CNProbs_A, 'G': CNProbs_G}

    # Calculate the average of CN2 probabilities
    CN2Average = calculateCN2ProbsAverage(CNProbs, chromType, exonStart, exonEnd, sampleName)

    # Check if the CN2 average is not zero
    if CN2Average != 0:
        # Calculate the quality score
        qualityScore = pathProb / CN2Average
    else:
        # Set quality score to zero if CN2 average is zero
        qualityScore = 0
...
def calculateCN2ProbsAverage(CNProbs, chromType, exonStart, exonEnd, sampleName):
    # Select the appropriate dictionary based on chromosome type
    CNProbsSelected = CNProbs[chromType]

    # Retrieve the probability array for the specified sample
    probs = CNProbsSelected[sampleName]
