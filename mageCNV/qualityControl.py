...
# Warning: this module uses another module,
# cleanest method I could find without defining the user's path.
import sys
import os
...
def SampsQC(counts, SOIs, windowSize):
    ...
    for sampleIndex in range(len(SOIs)):
        ...
        # using a sliding window to extract the window index "smallestIndex"
        # with the first minimum sum
        # density is observed "smallestSum" and a list 'densitiesSums',
        # with each density sum for each window browsed
        ...
        biggestIndex = densitiesSums.index(biggestSum) + windowSize // 2  # to remove not use only for dev

        listtest.append([SOIs[sampleIndex], smallestIndex, smallestSum, biggestIndex, biggestSum])
        ...
        if (((biggestSum - smallestSum) / biggestSum) < 0.20):
    ...
    logger.info("Uncovered exons number %s to be deleted before clustering for %s/%s valid samples.", 
                len(exons2RMAllSamps), validSampsCounter, len(SOIs))
    return(validityStatus, exons2RMAllSamps, listtest)
