...
def ResultsListInitialisation(sample,chrom,pos,key,end,CN,BF,RR,listS):
    ...
    #the value under analysis comes from a duplicated CNV.
        ...
        ValueToAdd=("0/1:"+int(CN)+":"+str(round(float(BF),2))+":"+str(round(float(RR),2)))
        indexToKeep=listS.index(sample)
        listResults[indexToKeep]=ValueToAdd  
...
def main(argv):
    ...
    logger.info("2) CNVs Number obtained after attribution copy number = %s/%s",len(CNVTableAllFilter),len(tabToTreat))
        ...
        for i in value:
            ...
            if len(firstline)==0:                
                ...
                #add information to the second list
                    ...
                    logger.error("Unable to treat the CNV identified at the position:"+key+" for the patient:"+i+" . Please check the correct format of the latter.")
                    sys.exit()
