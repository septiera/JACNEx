...
logger.propagate=False #Limits duplication of returned messages
...
def main(argv):
    ...
    for opt, value in opts:
        if opt == '-h':
            ...
            +"\n"
            +"\n OPTIONS:"
            +"\n	-b : indicates the path to the canonicalTranscript.bed file."
            +"\n	-n : A string is expected to describe the reference genome version."
            +"\n	-o : Path where create the new output file for the current analysis.")
            sys.exit()
    ...
    BedToComplete=isc.BedSanityCheck(BedCanonicalTranscript)
    ...
    BedToComplete.to_csv(os.path.join(PathStaticFolder,"STEP0_"+GenomeVersion+"_Padding10pb_"+str(len(BedToComplete))+"exons_"+str(now)+".bed"),index=False, header=False,sep="\t")
